...
add_filter( 'pre_http_request', function( $pre, $parsed_args, $url ) use ( $consumer_data ) {
    ...
    if ( strpos( $url, 'https://wp-rocket.me/valid_key.php' ) !== false ) {
        return generate_response( true, $consumer_data );
    }

    // Check for user statistics
    if ( strpos( $url, 'https://wp-rocket.me/stat/1.0/wp-rocket/user.php' ) !== false ) {
    ...
    return $pre; // Return the original request
}, 10, 3 );
